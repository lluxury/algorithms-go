...
（leetcode测试通过）
    ...
    * 双向链表：
        ...
        3. 所以，移除无用数据的时候，请删除头结点的下一个节点；添加新节点的时候，添加到尾节点的前一个；更新一个存储的节点的生命的时候，先移除他，然后再添加他（即自动的到最后去了）
    * 业务：
        * get数据：更新数据的生命：删除并添加节点
        * set数据：如果存在，则更新数据租期，并修改map的值；如果没有，则添加数据（链表+map），如果长度超出，删除最旧的数据（头结点的下一个节点）
*/
type Link struct {
	k    int
	v    int
